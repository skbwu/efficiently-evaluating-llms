...
# settings that we will get test results for on this specific coverage-analysis audit.
...
# which columns are we interested in for our logs? we know we're looking at fully-observed versions only
columns = ["dataset", "prop_budget", "seed"] + [str(i) for i in range(2213)]

# create a log file to store in our "logs" directory
logs_fname = f"coverage_analysis_dataset={dataset}.csv"
if logs_fname not in os.listdir("logs/coverage_analysis"):
    with open(f"logs/coverage_analysis/{logs_fname}", "w") as file:
...
def trial(
    ...
    coverages = (((lb <= mus_M2) & (mus_M2 <= ub))).flatten().cpu().numpy().astype(bool)
    
    # return our outputs -- here, that's just our 2212 or 2213 indicators
    return coverages


# how many settings do we have total?
NUM_SETTINGS = len(MISSINGNESS_SETTINGS) * len(BUDGET_PROPS) * len(SEED_LIST)

# a counter to check our progress
counter = 0

# checkpointing to see how many settings we've already finished.
checkpoint_counter = len(pd.read_csv(f"logs/coverage_analysis/{logs_fname}").index)
    ...
    for budget_prop in BUDGET_PROPS:
        ...
        for seed in SEED_LIST:
            ...
            if counter >= checkpoint_counter:
                ...
                # check length + pad a NaN if necessary.
                coverages = list(coverages)
                coverages = coverages if len(coverages) == 2213 else coverages + [np.nan]
                
                # create our row to write to our .csv
                row = [dataset, budget_prop, seed] + coverages
                
                # write to this .csv manually to avoid pandas overhead.
                with open(f"logs/coverage_analysis/{logs_fname}", "a") as file:
