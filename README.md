...
This repository accompanies the paper "Efficiently Evaluating LLM Performance with Statistical Guarantees."
...
**Workflow for reproducing all main text and appendix results and figures (except for cold-start results):**
...
**Workflow for reproducing cold-start results:** within the `cold_start` subdirectory,
1. Combining of MMLU-Pro and GPQA (All) is performed in `1. Data.ipynb`. You may need to copy raw data from the main `data` directory.
2. LLM embeddings for the above datasets' questions are performed in `2. Computing LLM Embeddings.ipynb` (requires OpenAI API key), with cold-start GPQA (all) model and question factors constructed in `3. Cold-Start Initializing Model Factor for GPQA (All).ipynb`.
3. FAQ hyperparameters were tuned in `4. Validating Cold-Start FAQ on GPQA (All).ipynb`, and FAQ final runs were performed on `5. Final Cold-Start FAQ on GPQA (All).ipynb`.
4. All baselines were run via `6. All Baselines for GPQA (All).ipynb` with post-hoc selection and summaries of all methods performed in `7. Analyzing Cold-Start FAQ vs. Baselines on GPQA (All).ipynb`.
5. Main text and appendix figures for cold-start FAQ were generated using `8. Visualizing Results.ipynb`.
