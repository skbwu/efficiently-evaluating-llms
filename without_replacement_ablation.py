...
# settings that we will do this without-replacement ablation for.
dataset = "mmlu-pro"
...
# main switch - are we going to do with/without replacement on this ablation?
WITHOUT_REPLACEMENT = [True, False][int(sys.argv[1])]

# which columns are we interested in for our logs?
columns = [
    "dataset", "prop_budget", "seed", 
    "mean_width", "coverage", "sq_err", "n_resampled", "n_resampled_unique"]

# create a log file to store in our "logs" directory
logs_fname = f"faq_without-replacement={WITHOUT_REPLACEMENT}.csv"
if logs_fname not in os.listdir("logs/final/without_replacement_ablation"):
    with open(f"logs/final/without_replacement_ablation/{logs_fname}", "w") as file:
...
def trial(
    ...
    # initialize counters of how many times I've sampled each question
    obs_counters = torch.zeros(size=(N_NEW, N_QUESTIONS)).to(device)
    ...
    for s in tqdm(range(N_B), desc=f"{str(counter).zfill(5)}"):
        ...
        # ABLATION STUDY - POTENTIALLY SAMPLING WITHOUT REPLACEMENT
        if WITHOUT_REPLACEMENT:

            # zero out the entries that have been sampled already + normalize again
            q_js[obs_counters > 0] = 0.0
            q_js = q_js / q_js.sum(dim=1, keepdim=True)
        ...
        #### PART 4: COUNTING WHICH QUESTIONS HAVE BEEN SAMPLED 

        # update our obs_counters
        obs_counters.scatter_add_(dim=1, index=I_s, src=torch.ones_like(I_s, dtype=obs_counters.dtype))
    ...
    # also get sq_err, num_resampled, n_resampled_unique
    sq_err = ((mus_M2 - thetahats_T) ** 2).mean().item()
    num_resampled = (obs_counters - 1).clamp(min=0.0).sum(dim=1).mean(dtype=float).item()
    n_resampled_unique = (obs_counters > 1).sum(dim=1).mean(dtype=float).item()

    # return our outputs
    outputs = [mean_width, coverage, sq_err, num_resampled, n_resampled_unique]
    return outputs

# how many settings do we have total?
NUM_SETTINGS = len(BUDGET_PROPS) * len(SEED_LIST)

# a counter to check our progress
counter = 0

# checkpointing to see how many settings we've already finished.
checkpoint_counter = len(pd.read_csv(f"logs/final/without_replacement_ablation/{logs_fname}").index)
...
U = torch.load(
    f"factor_models/final/{dataset}/U_nfobs=None_p=1.0.pt", map_location=device).to(device)
V = torch.load(
    f"factor_models/final/{dataset}/V_nfobs=None_p=1.0.pt", map_location=device).to(device)
D = U.shape[1]
...
for budget_prop in BUDGET_PROPS:
    ...
    # what is the best setting for the fully-observed dataset 
    ...
    for seed in SEED_LIST:
        ...
        if counter >= checkpoint_counter:
            ...
            row = [dataset, budget_prop, seed] + outputs

            # write to this .csv manually to avoid pandas overhead.
            with open(f"logs/final/without_replacement_ablation/{logs_fname}", "a") as file:
